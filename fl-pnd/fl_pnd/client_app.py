...
#         """在本地评估模型，并返回包含所有指标的字典。"""
#         set_parameters(self.net, parameters)
        
#         # test 函数现在返回 loss 和一个完整的 metrics 字典
#         loss, metrics_dict = test(net=self.net, testloader=self.valloader, device=DEVICE)
        
#         # [核心修改] 直接将 test 函数返回的 metrics 字典作为第三个返回值
#         # Flower 服务器端的通用 weighted_average 函数会自动处理这个字典
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # 从服务器传递过来的 config 字典中获取当前轮数
        current_round = config["server_round"]
        
        print(f"--- 客户端 {self.cid} 开始训练 (设备: {DEVICE}, 轮次: {current_round}) ---")
        train(
            net=self.net, 
            trainloader=self.trainloader, 
            epochs=1, # 坚持使用 1 个本地 epoch
            device=DEVICE, 
            class_weights=self.class_weights,
